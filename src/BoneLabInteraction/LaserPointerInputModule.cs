...
namespace UnityExplorer
{
    ...
    public class LaserPointerInputModule : BaseInputModule {
        ...
        public static void Process2(BaseInputModule __instance)
        {
            MelonLogger.Msg("module : " + __instance.GetActualType().Name);
            MelonLogger.Msg("module2 : " + __instance.GetType().Name);
            if (__instance.GetActualType() == typeof(LaserPointerInputModule))
            {
                LaserPointerInputModule t = (LaserPointerInputModule)__instance;
                t.raycaster.eventMask = t.layerMask;

                foreach (var pair in t._controllerData)
                {
                    IUILaserPointer controller = pair.Key;
                    ControllerData data = pair.Value;

                    // Test if UICamera is looking at a GUI element
                    t.UpdateCameraPosition(controller);
                    ...
                    data.pointerEvent.position = new Vector2(t.UICamera.pixelWidth * 0.5f, t.UICamera.pixelHeight * 0.5f);
                    //data.pointerEvent.scrollDelta = Vector2.zero;

                    // trigger a raycast
                    t.eventSystem.RaycastAll(data.pointerEvent, t.m_RaycastResultCache);
                    data.pointerEvent.pointerCurrentRaycast = FindFirstRaycast(t.m_RaycastResultCache);
                    t.m_RaycastResultCache.Clear();
                    ...
                    t.HandlePointerExitAndEnter(data.pointerEvent, data.currentPoint);
                    ...
                    {
                        data.pointerEvent.current = t.eventSystem.currentSelectedGameObject;
                        ExecuteEvents.Execute(t.eventSystem.currentSelectedGameObject, t.GetBaseEventData(), ExecuteEvents.updateSelectedHandler);
